...
func NewPGContainer(t *testing.T) *dockertest.Container {
	container, err := dockertest.RunContainer("postgres:alpine", "5432", func(addr string) error {
		db, err := sql.Open("postgres", "postgresql://postgres:postgres@"+addr+"?sslmode=disable")
		fatalErr(t, err)
...
func NewMockDB(t *testing.T, models ...interface{}) *pg.DB {
	db, err := postgres.New("postgresql://ranjeet:3298@localhost:5432/mockExample?sslmode=disable", 10, false)
	fatalErr(t, err)
...
func DeleteNewMockDB(t *testing.T, models ...interface{}) {
	db, err := postgres.New("postgresql://ranjeet:3298@localhost:5432/mockExample?sslmode=disable", 10, false)
	fatalErr(t, err)

	for _, v := range models {
		fatalErr(t, db.DropTable(v, &orm.DropTableOptions{IfExists: true}))
	}
